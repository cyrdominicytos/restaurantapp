...
type RestOf<T extends IProduct | NewProduct> = Omit<T, 'createdDate' | 'updatedDate'> & {
...
export class ProductService {
  ...
  protected convertDateFromClient<T extends IProduct | NewProduct | PartialUpdateProduct>(product: T): RestOf<T> {
    return {
      ...
      updatedDate: product.updatedDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restProduct: RestProduct): IProduct {
    return {
      ...
      updatedDate: restProduct.updatedDate ? dayjs(restProduct.updatedDate) : undefined,
    };
  }
