...
describe('Product e2e test', () => {
  ...
  const productSample = { price: 25543.39, name: 'maigre bien que' };
  ...
  describe('new Product page', () => {
    ...
    it('should create an instance of Product', () => {
      cy.get(`[data-cy="price"]`).type('3368.8');
      cy.get(`[data-cy="price"]`).should('have.value', '3368.8');

      cy.get(`[data-cy="name"]`).type('ouah que vroum');
      cy.get(`[data-cy="name"]`).should('have.value', 'ouah que vroum');

      cy.setFieldImageAsBytesOfEntity('photo', 'integration-test.png', 'image/png');

      cy.get(`[data-cy="imageType"]`).type('prout en dedans de');
      cy.get(`[data-cy="imageType"]`).should('have.value', 'prout en dedans de');

      cy.get(`[data-cy="description"]`).type('sur');
      cy.get(`[data-cy="description"]`).should('have.value', 'sur');

      cy.get(`[data-cy="isSupplement"]`).should('not.be.checked');
      cy.get(`[data-cy="isSupplement"]`).click();
      cy.get(`[data-cy="isSupplement"]`).should('be.checked');

      cy.get(`[data-cy="createdDate"]`).type('2024-02-01T15:16');
      cy.get(`[data-cy="createdDate"]`).blur();
      cy.get(`[data-cy="createdDate"]`).should('have.value', '2024-02-01T15:16');

      cy.get(`[data-cy="updatedDate"]`).type('2024-02-01T21:54');
      cy.get(`[data-cy="updatedDate"]`).blur();
      cy.get(`[data-cy="updatedDate"]`).should('have.value', '2024-02-01T21:54');
