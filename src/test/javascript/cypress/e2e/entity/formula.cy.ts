...
describe('Formula e2e test', () => {
  ...
  const formulaSample = { price: 14446.41 };
  ...
  describe('new Formula page', () => {
    ...
    it('should create an instance of Formula', () => {
      cy.get(`[data-cy="price"]`).type('18710.47');
      cy.get(`[data-cy="price"]`).should('have.value', '18710.47');

      cy.get(`[data-cy="name"]`).type('collègue boum');
      cy.get(`[data-cy="name"]`).should('have.value', 'collègue boum');

      cy.setFieldImageAsBytesOfEntity('photo', 'integration-test.png', 'image/png');

      cy.get(`[data-cy="imageType"]`).type('exprès secours');
      cy.get(`[data-cy="imageType"]`).should('have.value', 'exprès secours');

      cy.get(`[data-cy="description"]`).type('circulaire');
      cy.get(`[data-cy="description"]`).should('have.value', 'circulaire');

      cy.get(`[data-cy="createdDate"]`).type('2024-02-01T12:02');
      cy.get(`[data-cy="createdDate"]`).blur();
      cy.get(`[data-cy="createdDate"]`).should('have.value', '2024-02-01T12:02');

      cy.get(`[data-cy="updatedDate"]`).type('2024-02-01T14:54');
      cy.get(`[data-cy="updatedDate"]`).blur();
      cy.get(`[data-cy="updatedDate"]`).should('have.value', '2024-02-01T14:54');
