...
describe('Product e2e test', () => {
  ...
  const productSample = { price: 75443, name: 'Handcrafted drive invoice' };
  ...
  describe('new Product page', () => {
    ...
    it('should create an instance of Product', () => {
      cy.get(`[data-cy="price"]`).type('24195').should('have.value', '24195');

      cy.get(`[data-cy="name"]`).type('Program Massachusetts Personal').should('have.value', 'Program Massachusetts Personal');

      cy.setFieldImageAsBytesOfEntity('photo', 'integration-test.png', 'image/png');

      cy.get(`[data-cy="imageType"]`).type('transition Division monitor').should('have.value', 'transition Division monitor');

      cy.get(`[data-cy="description"]`).type('SCSI Rupee').should('have.value', 'SCSI Rupee');

      cy.get(`[data-cy="isSupplement"]`).should('not.be.checked');
      cy.get(`[data-cy="isSupplement"]`).click().should('be.checked');

      cy.get(`[data-cy="createdDate"]`).type('2024-02-01T13:00').blur().should('have.value', '2024-02-01T13:00');

      cy.get(`[data-cy="updatedDate"]`).type('2024-01-31T22:42').blur().should('have.value', '2024-01-31T22:42');
