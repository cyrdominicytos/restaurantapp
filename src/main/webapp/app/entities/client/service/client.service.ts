...
type RestOf<T extends IClient | NewClient> = Omit<T, 'createdDate' | 'updatedDate'> & {
...
export class ClientService {
  ...
  protected convertDateFromClient<T extends IClient | NewClient | PartialUpdateClient>(client: T): RestOf<T> {
    return {
      ...
      updatedDate: client.updatedDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restClient: RestClient): IClient {
    return {
      ...
      updatedDate: restClient.updatedDate ? dayjs(restClient.updatedDate) : undefined,
    };
  }
